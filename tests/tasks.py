...
def test_task(task: str) -> None:
    ...
    # Check script
    script_errors = check_script_blocks(task_info[task]["script_path"])
    if script_errors:
    ...
    with ActiveLogger(logger):
        ...
        if e.value.code != 0:
            logger.fatal(f"Core exited with non-zero code for task '{task}'")
        ...
        if successful_tasks:
            logger.padded_print(f"Successful tasks ({len(successful_tasks)})")
