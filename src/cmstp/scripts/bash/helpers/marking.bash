...
_insert_around_marker() {
  ...
  if [ "$position" == "before" ]; then
    ...
    log_step "(write_around_mark) Invalid position: $position" true
    return 1
  fi

  # Use sed to insert the message
  sed -i "/$marker/$position_ch\\
$msg
" "$file"
}
...
write_marked() {
  ...
  if [ "$check_existing" = true ] && grep -Fq "$content" "$file"; then
    log_step "'$content' already exists in $file - Skipping"
    return 0
  fi
