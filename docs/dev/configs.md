# Package Configs
Main configuration files are in `src/gurk/config/`:
...
```bash
gurk info [-c|--custom-config] [-d|--default-config]
```

# Config directory
Some tasks may use their own config files for lists of packages, settings, etc. These should be stored under `src/gurk/config/<command>` for the resp. task command.
...
Please also open a PR (or contact the maintainers) to add this file to this package's sister repository, [`gurk_config`](https://github.com/ArturoRoberti/gurk_config).
